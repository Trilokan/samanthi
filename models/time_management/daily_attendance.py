...
class DailyAttendance(models.Model):
    ...
    def get_report(self):
        ...
        self.week_off_count = rec_obj.search_count([("day_progress", "=", "holiday"),
                                                    ("attendance_id", "=", self.id)])
        self.working_count = rec_obj.search_count([("day_progress", "=", "working_day"),
                                                   ("attendance_id", "=", self.id)])
        self.present = rec_obj.search_count([("availability_progress", "=", "full_day"),
                                             ("attendance_id", "=", self.id)])
        self.half_day_present = rec_obj.search_count([("availability_progress", "=", "half_day"),
                                                      ("attendance_id", "=", self.id)])
        self.absent = rec_obj.search_count([("availability_progress", "=", "absent"),
                                            ("attendance_id", "=", self.id)])
    ...
    _sql_constraints = [('unique_attendance', 'unique (date)', 'Error! Date should not be repeated')]
